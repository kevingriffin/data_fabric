...
class DynamicSwitchingTest < Test::Unit::TestCase
  ...
  def test_reads_from_slave_when_below_threshold
    ...
    assert_equal "df_test_slave", ReplicateModel.find(1).name
  end

  def test_reads_from_master_when_above_threshold
    ReplicateModel.connection.status_checker.poller = PollerMock.new(true)
    assert_equal "df_test_master", ReplicateModel.find(1).name
  end

  def test_with_master_always_goes_to_master
    ReplicateModel.connection.status_checker.poller = PollerMock.new(false)
    assert_equal "df_test_master", ReplicateModel.with_master() { ReplicateModel.find(1).name }
  end
  ...
  def test_with_master_can_be_nested
    ...
    assert_equal "df_test_masterdf_test_masterdf_test_master", result
  end
  ...
  def test_with_slave_can_be_nested
    ...
    assert_equal("df_test_masterdf_test_slavedf_test_slavedf_test_master", result, "Slave should not swap when in a fixed role block.")
  end
  ...
  def test_find_in_batches_doesnt_swap_during_a_find_when_inside_current_db
    ...
    ReplicateModel.with_current_db { ReplicateModel.find_in_batches(:batch_size => 1) { |batch| sleep 0.5; assert_equal "df_test_slave", batch.first.name } }
  end
end
